...
export class ProjectHouse extends Phaser.Scene {
...
this.dpad1 = new DPad(this, 640, 440, 25, this.inputFlags);
    ...
    const map = this.make.tilemap({ key: "projecthouse" });
    const tileset = map.addTilesetImage("Museum interior", "projecttiles");
...
this.me.setCollideWorldBounds(true);
    ...
    doorLayer?.objects.forEach((obj: any) => {
      ...
      this.physics.add.overlap(this.me, doorZone, () => {
        ...
        const spawnX = obj.properties?.find(
          (p: any) => p.name === "spawnx"
        )?.value;
        const spawnY = obj.properties?.find(
          (p: any) => p.name === "spawny"
        )?.value;
