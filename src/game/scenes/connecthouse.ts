...
export class ConnectHouse extends Phaser.Scene {
...
this.dpad1 = new DPad(this, 640, 440, 25, this.inputFlags);
   ...
   const map = this.make.tilemap({ key: "connecthouse" });
    const tileset = map.addTilesetImage("Poke Centre interior", "connecttiles");
...
doorLayer?.objects.forEach((obj: any) => {
  ...
  this.physics.add.overlap(this.me, doorZone, () => {
    ...
    const spawnX = obj.properties?.find((p: any) => p.name === "spawnx")?.value || 100;
    const spawnY = obj.properties?.find((p: any) => p.name === "spawny")?.value || 100;
